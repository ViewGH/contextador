...
test("finds matching hash", () => {
  const events = [
    { content: buildBroadcast({ query: "auth flow", queryHash: "abc123", scopes: ["s/gateway"], pointers: { p: 1 }, tokensUsed: 100, agentId: "ctx-1", contextValidated: "sha1" }) },
    { content: buildBroadcast({ query: "data pipeline", queryHash: "def456", scopes: ["s/backend"], pointers: { p: 2 }, tokensUsed: 200, agentId: "ctx-2", contextValidated: "sha2" }) },
  ];
