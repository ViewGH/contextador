...
test("routeQuery short-circuits to professor for specific scope", async () => {
  ...
  await writeFile(join(TEST_ROOT, "services/backend/CONTEXT.md"), "# Backend\n## Internal Structure\n- pipelines/ — Processing pipelines including ingestion, validation");
  await writeFile(join(TEST_ROOT, "services/backend/pipelines/CONTEXT.md"), "# Pipelines\n- processing/ — Document processing");
  await writeFile(join(TEST_ROOT, "services/backend/pipelines/processing/CONTEXT.md"), "# Processing Pipeline\n## Purpose\nProcesses incoming documents through 5 stages.");

  const route = await routeQuery(TEST_ROOT, "how does document processing work?");

  expect(route.targetScope).toContain("processing");
