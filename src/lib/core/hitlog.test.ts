...
test("hashKeywords produces consistent hash", () => {
  const h1 = hashKeywords(["order", "processing", "failed"]);
  const h2 = hashKeywords(["order", "processing", "failed"]);
  expect(h1).toBe(h2);
});

test("hashKeywords is order-independent", () => {
  const h1 = hashKeywords(["order", "processing"]);
  const h2 = hashKeywords(["processing", "order"]);
  expect(h1).toBe(h2);
});

test("hashKeywords different for different keywords", () => {
  const h1 = hashKeywords(["order", "processing"]);
  const h2 = hashKeywords(["auth", "gateway"]);
