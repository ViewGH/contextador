...
test("detectImports finds Python imports (from/import)", async () => {
  ...
  await writeFile(
    ...
    [
      "from services.backend.pipelines import eob_handler",
...
test("matchImportsToScopes maps to known scopes correctly", () => {
  const knownScopes = [
    ...
    "services/backend/pipelines/eob",
    "tools/contextador",
  ];

  const imports = [
    "services.backend.pipelines.eob.handler",   // Python-style → services/backend/pipelines/eob
  ...
  expect(matched).toContain("services/backend/pipelines/eob");
